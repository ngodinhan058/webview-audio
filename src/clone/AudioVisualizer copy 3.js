...
export default function AudioVisualizer({ audioUrl }) {
  useEffect(() => {
    const unlockAudio = () => {
      ...
      dummy.play().catch(() => {}); // play một cái audio rỗng để unlock
      dummy.pause();
  ...
  const startVisualizer = useCallback(() => {
    ...
    renderer.domElement.style.top = '-200px'; // đổi tuỳ ý
    ...
    const material = new THREE.MeshLambertMaterial({ color: "#616161", wireframe: true });
  ...
  document.addEventListener('message', async (event) => {
    try {
      ...
      if (message.type === 'audio') {
        ...
        // const blob = await fetch(`data:audio/webm;base64,${base64}`).then(r => r.blob());
        // const file = new File([blob], "recording.webm", { type: "audio/webm" });
        // uploadFormData.append("file", file);

        // const uploadRes = await fetch("https://your-api/upload", {
        //   method: "POST",
        //   headers: {
        //     // Không cần Content-Type — browser sẽ set cho multipart/form-data
      ...
      console.error("Invalid message or upload error:", err);
    }
  });
}, []);
